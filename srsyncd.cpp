...
DWORD WINAPI handle_query(void *lp) {
	int &sock_fd = *(int *)lp;
#else
int handle_query(string executable_path, string options, int sock_fd, string policy_d, string log_fn, int log_level, struct in_addr sin_addr) {
	...
	if(credentials != NULL)
		command << executable_path << options << " \"prove_c('" << policy_d << "/" << string(query) << ".policy', " << string(principal) << ", " << string(credentials) << ", " << string(query) << ")\" 2>&1";
	else
		command << executable_path << options << " \"prove_c('" << policy_d << "/" << string(query) << ".policy', " << string(principal) << ", " << string(query) << ")\" 2>&1";
		...
		if((new_sock_fd = accept(sock_fd, (struct sockaddr *)&cli_addr, (socklen_t *)&cli_len)) != -1) {
			...
			switch(pid = fork()) {
				...
				case 0:
					...
					return handle_query(executable_path, options, new_sock_fd, policy_d, log_fn, log_level, cli_addr.sin_addr);
				default:
					close(new_sock_fd);
			}
#endif
