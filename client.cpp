...
vector< vector< vector<string> * > * > split_or(string s) {
	vector< vector< vector<string> * > * > orv;
	size_t p = 0;
	for(size_t i = 0; i < s.size(); i++) {
		if(s[i] == '{') p = i + 1;
		else if(s[i] == ';') { orv.push_back(split_multiple(s.substr(p, i - p))); p = i + 1; }
		else if(s[i] == '}') { orv.push_back(split_multiple(s.substr(p, i - p))); p = i + 1; }
	}
	return orv;
}
...
int process_query(string whoami, string private_key_fn, string public_keys_rn, string queried_party, string credentials, string query, map<string, string> addresses, bool interactive, map<pair<string, size_t>, bool> history) {
	...
	if(answer_metadata.status == FAILURE) {
		...
		vector< vector< vector<string> * > * > vor = split_or(answer);
		stringstream new_credentials; new_credentials << "(" << credentials << ")";
		size_t ior = 0, successesor = 0;
		if(interactive) {
			unsigned int *or_choices_index = (unsigned int *)malloc(vor.size() * sizeof(unsigned int));
			memset(or_choices_index, 0, vor.size() * sizeof(unsigned int));
			...
			do {
				...
				for(size_t j = 0; j < vor.size(); j++) {
					if(!or_choices_index[j]) {
						or_choice_banner << j << ". ";
						for(size_t k = 0; k < vor[j]->size(); k++) {
							or_choice_banner << "[";
							for(size_t l = 0; l < (*vor[j])[k]->size() - 1; l++)
								or_choice_banner << (*(*vor[j])[k])[l] << ", ";
							or_choice_banner << (*(*vor[j])[k])[(*vor[j])[k]->size() - 1];
							or_choice_banner << "]";
						}
						or_choice_banner << endl;
					}
				}
				if(or_choices) {
					cout << or_choice_banner.str();
					int or_c = -1;
					do {
						cout << "> ";
						while(!(cin >> or_c)) {
					...
					} while(or_c < 0 || or_c >= (int)vor.size() || or_choices_index[or_c]);
					or_choices_index[or_c] = 1;
					or_choices--;
					vector< vector<string> * > &vm = *vor[or_c];
					...
					if(vm.size() == successes) successesor++;
				}
			} while(!successesor && or_choices);
			free(or_choices_index);
		} else {
			while(ior < vor.size() && !successesor) {
				vector< vector<string> * > &vm = *vor[ior];
				size_t i = 0, successes = 0;
				while(i < vm.size() && i == successes) {
					vector<string> *v = vm[i];
					if(!already_granted(*v, i, history)) {
...
void process_requests(string whoami, string private_key_fn, string public_keys_rn, const char *fn, string credentials, map<string, string> addresses, bool interactive, map<pair<string, size_t>, bool> history) {
	ifstream f(fn);
	if(!f.is_open()) {
	...
	while(f.good()) {
		getline(f, line);
		if((p = line.find("@")) != string::npos) {
			process_query(whoami, private_key_fn, public_keys_rn, line.substr(line.find_first_not_of(" \t", p + 1), line.find_last_not_of(" \t") - line.find_first_not_of(" \t", p + 1) + 1), credentials, line.substr(0, p), addresses, interactive, history);
		} else if(line != "") {
...
int main(int argc, char *argv[]) {
	...
	if(credentials->count > 0)
		process_requests(whoami->sval[0], prikey->filename[0], pubkeys->filename[0], request->filename[0], process_credentials(credentials->filename[0]), process_addresses(addresses->filename[0]), interactive->count > 0, history);
	else
		process_requests(whoami->sval[0], prikey->filename[0], pubkeys->filename[0], request->filename[0], "", process_addresses(addresses->filename[0]), interactive->count > 0, history);
